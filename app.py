                ...
                for row in range(len(myr)):
                    if myr[row][0] == login and myr[row][1] == date and myr[row][2] == family:
                        ...
                        if row == len(myr) - 1:
                            mydb = sqlite3.connect('base.sqlite')
                            mycursor = mydb.cursor()
                            vstavka = "INSERT INTO оценки ('предмет', 'дата', 'фамилия', 'оценка') VALUES (?,?,?,?)"
...
def raspisanie():
    if request.method == 'POST':
        ...
        myresult = mycursor.fetchall()
        if len(myresult) == 0:
            sqlFormula = "INSERT INTO расписание ('понедельник', 'вторник', 'среда', 'четверг', 'пятница', 'суббота') VALUES (?,?,?,?,?,?)"
            ...
            mycursor.execute('DELETE FROM расписание')
            sqlFormula = "INSERT INTO расписание ('понедельник', 'вторник', 'среда', 'четверг', 'пятница', 'суббота') VALUES (?,?,?,?,?,?)"
            mycursor.execute(sqlFormula, (d1l1, d2l1, d3l1, d4l1, d5l1, d6l1))
            mycursor.execute(sqlFormula, (d1l2, d2l2, d3l2, d4l2, d5l2, d6l2))
            mycursor.execute(sqlFormula, (d1l3, d2l3, d3l3, d4l3, d5l3, d6l3))
...
def add_tema():
    if request.method == 'POST':
        ...
        sqlFormula = "INSERT INTO темы ('дата', 'тема', 'классная', 'домашняя', 'предмет') VALUES (?,?,?,?,?)"
...
def raspol_uchenik():
    if request.method == 'POST':
        ...
        mydb = sqlite3.connect('base.sqlite')
        mycursor = mydb.cursor()
        mycursor.execute('DELETE FROM рассадка')
        mydb.commit()
        mydb = sqlite3.connect('base.sqlite')
        mycursor = mydb.cursor()
        mycursor.execute('SELECT * FROM рассадка')
        myresult = mycursor.fetchall()
        sqlFormula = "INSERT INTO рассадка ('1ряд', '2ряд', '3ряд') VALUES (?,?,?)"
...
if __name__ == '__main__':
    app.run(debug=True, port=7070, host='127.0.0.1')
